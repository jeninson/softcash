...
try {
    if($_SERVER['REQUEST_METHOD'] == 'GET') {
        ...
        try {
            ...
            $_POST = json_decode(file_get_contents('php://input'), true);
            // Validar parámetros
            if (!empty($_POST['id_reg_act']) && !empty($_POST['idTk']) && !empty($_POST['idUser']) && !empty($_POST['tipo_registro']) && !empty($_POST['fecha']) && !empty($_POST['valor']) && !empty($_POST['categoria']) && !empty($_POST['descripcion'])) {
                $id_reg_act = htmlspecialchars(trim($_POST['id_reg_act']));
                $idTk = htmlspecialchars(trim($_POST['idTk']));
                $idUser = htmlspecialchars(trim($_POST['idUser']));
                $tipo_registro = htmlspecialchars(trim($_POST['tipo_registro']));
                $fecha = htmlspecialchars(trim($_POST['fecha']));
                ...
                $registrosModel = new RegristrosModel();
                //Llamada al metodo de autenticacion
                $result = $registrosModel->actualizarRegistro($id_reg_act, $idUser, $tipo_registro, $fecha, $valor, $idcategoria, $descripcion);
                if ($result) {
                    header("HTTP/1.1 200 OK");
                    echo json_encode(array("code"=>200, "msg" => "Registro guardado correctamente"));
                } else {
